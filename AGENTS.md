...
The framework follows a "Hybrid Glue" pattern where Python handles orchestration and high-level logic, while C/C++ handles performance-critical and low-level systems interaction.
...
- **`bootstrap.py`**: The main entry point. It uses `FrameworkLoader` to dynamically discover Python modules and provide a shell for interaction.
