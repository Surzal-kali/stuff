...
- 2026-09-11 ~13:50 MT | amass keyed crypto.com run 2768a1d1 (subdomain_enum, passive, root config keys live) | user-approved first keyed spend; big-domain scan expected long-runtime; poll-verify then HOLD per user (sleep-deprived). Baseline for comparison: 124 clean CT names (crt.sh session, Sept 11 ~16-18h MT). Expect out_of_scope classification vs .scope manifest.
- 2026-09-11 ~15:1x MT | F-025 ROOT-CAUSED + PATCHED 3915675: amass v5 wrapper _retrieve_names had timeout=30 on `amass subs` dump + bare except swallowing TimeoutExpired -> multi-hour big-domain enums reported 0 names (2768a1d1: 2.5h crypto.com keyed run -> 0). Fix: AMASS_RETRIEVAL_TIMEOUT env (default 600s) + one retry; crt.sh fallback urlopen 45s -> AMASS_CRTSH_TIMEOUT (120s); empty-note now points at recoverable out_dir. Sandbox commit 3915675, py_compile OK, diff reviewed in chat, awaiting user SSH apply + framework restart (safe: 2768a1d1 done, nothing in flight). crypto.com data recoverable from desktop /tmp/amass_v5_* via amass subs.
